...
# Project notes
- Multi-tenant HR SaaS "Harbor" (byline "by Nuit Works"; all Nuit Works links via `nuitWorksUrl()` in `src/lib/brand.ts`). Brand/prices/trial/rates: `src/config/app.config.ts`. Tool registry: `src/modules/registry.ts` (single source of truth). UI says "tools" (code keeps "modules"); stages foundation/hire/run/pay/grow; Claims merged transport+expenses (claim_types). Setup = questions (`src/modules/setup-questions.ts`). Never copy MIWI naming/structure; banned words listed in project brief.
