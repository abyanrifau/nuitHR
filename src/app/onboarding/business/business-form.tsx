      ...
      <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent className="grid gap-4 sm:grid-cols-2">
            <Field label="Business name" htmlFor="name" error={err.name} className="sm:col-span-2">
              <Input id="name" name="name" value={v.name} onChange={set("name")} required aria-invalid={!!err.name} autoFocus={isNew} />
            </Field>
            <Field label="Industry" htmlFor="industry" error={err.industry} hint="We'll suggest the right modules for you.">
              <Select
                id="industry"
                name="industry"
                value={v.industry}
            ...
            <Field label="Number of employees (roughly)" htmlFor="employee_count_range" error={err.employee_count_range}>
              <Select
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Add each place your staff work, for example &quot;Malé Office&quot; or &quot;Resort Island&quot;.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            ...
            {branches.map((b, i) => (
              <div key={b.id ?? `new-${i}`} className="grid gap-2 sm:grid-cols-[1fr_1fr_auto]">
                ...
                />
                <Input
                  aria-label={`Branch ${i + 1} island or area`}
            ...
            <Button variant="secondary" size="sm" onClick={() => setBranches((l) => [...l, { name: "" }])}>
              <Plus className="size-4" aria-hidden /> Add another branch
            </Button>
            <p className="flex items-center gap-1.5 text-xs text-muted-foreground">
              <MapPin className="size-3.5" aria-hidden /> You can pin each branch on a map later for GPS clock-in.
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Used on letters and payslips. All optional for now.</CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Business email" htmlFor="email" optional error={err.email}>
      ...
      <div className="space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Shown on letters, payslips and the staff app.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <label
              htmlFor="logo"
        ...
        <div className="lg:sticky lg:top-6">
          <SubmitButton size="lg" className="w-full" pendingText="Saving…">
            {isNew ? "Create my workspace" : "Save and continue"}
          </SubmitButton>
          <p className="mt-2 text-xs text-subtle-foreground">Next: choose your modules</p>
        </div>
      </div>
    </form>
  );
